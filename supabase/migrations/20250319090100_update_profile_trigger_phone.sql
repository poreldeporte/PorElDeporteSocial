...
declare
  ...
  phone text := nullif(trim(coalesce((new.raw_user_meta_data ->> 'phone'), new.phone, '')), '');
  email text := nullif(trim(coalesce((new.raw_user_meta_data ->> 'email'), new.email, '')), '');
  full_name text := nullif(trim(coalesce((new.raw_user_meta_data ->> 'full_name'), '')), '');
begin
  insert into public.profiles (id, first_name, last_name, email, phone, name)
  values (
    new.id,
    first_name,
    last_name,
